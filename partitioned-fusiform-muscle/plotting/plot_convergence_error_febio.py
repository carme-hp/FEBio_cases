...
# Separate data by grid type
structured_mask = np.array([gt == "structured" for gt in grid_types])
unstructured_mask = np.array([gt == "unstructured" for gt in grid_types])

# Plot 1: L2 Error vs Number of Cells
if np.any(structured_mask):
    ax1.loglog(num_cells_array[structured_mask], l2_errors_array[structured_mask], 
               'bo-', linewidth=2, markersize=8, label='L2 Error (Structured Grid)')
if np.any(unstructured_mask):
    ax1.loglog(num_cells_array[unstructured_mask], l2_errors_array[unstructured_mask], 
               'rs-', linewidth=2, markersize=8, label='L2 Error (Unstructured Grid)')
...
if np.any(structured_mask):
    ax2.loglog(num_cells_array[structured_mask], relative_errors_array[structured_mask], 
               'bo-', linewidth=2, markersize=8, label='Relative Error (Structured Grid)')
if np.any(unstructured_mask):
    ax2.loglog(num_cells_array[unstructured_mask], relative_errors_array[unstructured_mask], 
               'rs-', linewidth=2, markersize=8, label='Relative Error (Unstructured Grid)')
