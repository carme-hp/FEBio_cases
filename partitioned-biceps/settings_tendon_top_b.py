...
# input mesh file
import os
input_directory   = os.path.join(os.environ["OPENDIHU_INPUT"])
fiber_file = input_directory + "/left_biceps_brachii_tendon2b.bin"        # bottom tendon
...
config_hyperelasticity = {    # for both "HyperelasticitySolver" and "DynamicHyperelasticitySolver"
  ...
  "OutputWriter" : [
    ...
    # Paraview files
    {"format": "Paraview", "outputInterval": variables.output_timestep, "filename": "out/" + variables.case_name + "/tendon_top_b", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
  ...
  "dynamic": {    # output of the dynamic solver, has additional virtual work values 
    "OutputWriter" : [   # output files for displacements function space (quadratic elements)
      # {"format": "Paraview", "outputInterval": variables.output_timestep, "filename": "out/tendon_top_b_virtual_work", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
...
config = {
  ...
  "PreciceAdapter": {        # precice adapter for bottom tendon
    ...
    "preciceParticipantName":   "TendonTopB",         # name of the own precice participant, has to match the name given in the precice xml config file
    ...
    "preciceSurfaceMeshes": [                                      # the precice meshes get created as the top or bottom surface of the main geometry mesh of the nested solver
      {
        "preciceMeshName":      "TendonTopBMesh",        # precice name of the 2D coupling mesh
    ...
    "preciceSurfaceData": [
      {
        ...
        "preciceMeshName":      "TendonTopBMesh",                 # name of the precice coupling surface mesh, as given in the precice xml settings file
      ...
      },
      {
        ...
        "preciceMeshName":      "TendonTopBMesh",                 # name of the precice coupling surface mesh, as given in the precice xml settings 
